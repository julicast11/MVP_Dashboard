...
// Sort by value descending for horizontal bar
const sortedInventory = [...inventoryByCategory].sort((a, b) => b.value - a.value);

const statusConfig = {
  critical: { label: 'Low Stock', bg: 'bg-red-50', text: 'text-red-700', icon: AlertCircle, dot: 'bg-red-500' },
  warning: { label: 'Watch', bg: 'bg-amber-50', text: 'text-amber-700', icon: AlertTriangle, dot: 'bg-amber-500' },
  overstock: { label: 'Overstock', bg: 'bg-blue-50', text: 'text-blue-700', icon: TrendingUp, dot: 'bg-blue-500' },
};
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KPICard
          ...
          value={k.turnoverRatio}
          prior={k.turnoverPrior}
          suffix="x"
          target={k.turnoverTarget}
          status={k.turnoverRatio >= k.turnoverTarget ? 'green' : 'yellow'}
        />
        <KPICard
          label="Days of Inventory on Hand"
          value={k.daysOnHand}
          prior={k.daysOnHandPrior}
          suffix=" days"
          invertColor
          status={k.daysOnHand > 90 ? 'red' : k.daysOnHand > 75 ? 'yellow' : 'green'}
        />
        <KPICard
          label="Stock Alerts"
          value={stockAlerts.filter(a => a.status === 'critical').length}
          suffix=" critical"
          status={stockAlerts.some(a => a.status === 'critical') ? 'red' : 'green'}
        />
      </div>

      {/* ── Middle Row ── */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Inventory Levels by Category — horizontal bar */}
        ...
        {/* Stock Flow — grouped bar (replaces dual-axis) */}
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
                ...
                return (
                  <tr key={item.sku} className={`${cfg.bg} hover:bg-opacity-80`}>
                    ...
                    <td className="px-5 py-3 text-right">
                      <span className={`font-medium ${pct < 50 ? 'text-red-600' : pct < 100 ? 'text-amber-600' : 'text-blue-600'}`}>
                        {pct}%
                      </span>
                    </td>
                  </tr>
