    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard
          ...
          value={s.netCashPosition}
          prior={s.priorPeriodCash}
          prefix="$"
          status="green"
        />
        <KPICard
          ...
          prefix="$"
          invertColor
          target={s.burnThreshold}
          status={s.burnRate > s.burnThreshold ? 'red' : s.burnRate > s.burnThreshold * 0.8 ? 'yellow' : 'green'}
        />
        <KPICard
          label="Accounts Payable"
          value={apSummary.totalPayable}
          prior={1_750_000}
          prefix="$"
          invertColor
          status={apSummary.overdue > 200_000 ? 'red' : apSummary.overdue > 100_000 ? 'yellow' : 'green'}
        />
        <KPICard
          label="30-Day Cash Forecast"
          value={s.projectedCash30Days}
          prior={s.netCashPosition}
          prefix="$"
          status="green"
        />
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={last30}>
              ...
              <Area
                type="monotone" dataKey="inflows" name="Inflows"
                stroke="#22c55e" fill="#22c55e" fillOpacity={0.15} strokeWidth={2}
              />
              <Area
                type="monotone" dataKey="outflows" name="Outflows"
                stroke="#ef4444" fill="#ef4444" fillOpacity={0.1} strokeWidth={2}
                strokeDasharray="4 3"
              />
            </AreaChart>
          </ResponsiveContainer>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
        ...
        <ResponsiveContainer width="100%" height={200}>
          <LineChart data={cashForecast}>
            ...
            <ReferenceLine y={2_500_000} stroke="#ef4444" strokeDasharray="5 5" label={{ value: 'Min Target', fontSize: 10, fill: '#ef4444' }} />
            <Line
              type="monotone" dataKey="projected" name="Projected Cash"
